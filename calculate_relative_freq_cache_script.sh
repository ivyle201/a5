...
echo "Reading hashtag files from directory $INFLUENCE_CLUSTER and writing to $output_file. Processing...wait time: $time."
...
for INFL_FILE in `ls $INFLUENCE_CLUSTER | head -n 1`; do
	for HASHTAG in `cat $INFLUENCE_CLUSTER/$INFL_FILE` ; do
		...
		echo $HASHTAG
		#echo $count_hashtags_in_C
		#echo  $frequency_H_overall
		#echo $count_hashtags_entire_dataset
		echo $count_H_entire_dataset
		frequency_H_in_C=`echo "scale=10; $count_H_in_C/$count_hashtags_in_C" | bc -l`
		frequency_H_overall=`echo "scale=10; $count_H_entire_dataset/$count_hashtags_entire_dataset" | bc -l`
		relative_frequency_H_C=`echo "scale=10; $frequency_H_in_C/$frequency_H_overall" | bc -l`
